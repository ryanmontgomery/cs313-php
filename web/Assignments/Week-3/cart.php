...
if (isset($_POST['PetID'])) {
    //TODO: set cart at ID to QUANTITY
    //var_dump($_POST);
    //var_dump($_SESSION['cart'][$_POST['PetID']]);
    $_SESSION['cart'][$_POST['PetID']] = intval($_POST['quantity']);
    if($_SESSION['cart'][$_POST['PetID']] == 0) {
        unset($_SESSION['cart'][$_POST['PetID']]);
    }
}
        ...
        <main>
            ...
            for ($i = 1; $i <= count($catalog); $i++) {
                if (isset($_SESSION['cart'][$i])) {
                    echo "<span>" . $catalog[$i]["Name"] . "</span>";
                    echo "<span> $" . $catalog[$i]["Price"] . "</span>";
                    echo "<form action='cart.php' method='post'>";
                    //TODO: set hidden input to id
                    echo "<input type='hidden' name='PetID' value='" . $catalog[$i]["PetID"] . "'>";
                    echo "<input type='number' name='quantity' value='" . $_SESSION['cart'][$i] . "'>";
                    echo "<input type='submit' value='Update'></form>";
                    
                    $itemTotal = $catalog[$i]["Price"] * $_SESSION['cart'][$i];
                    $total += $itemTotal;
                }
            }
